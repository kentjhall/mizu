...
std::optional<Key128> DeriveSDSeed() {
    const auto system_save_43_path =
        Common::FS::GetMizuPath(Common::FS::MizuPath::NANDDir) / "system/save/8000000000000043";
    ...
    const auto sd_private_path =
        Common::FS::GetMizuPath(Common::FS::MizuPath::SDMCDir) / "Nintendo/Contents/private";
...
KeyManager::KeyManager() {
    // Initialize keys
    const auto mizu_keys_dir = Common::FS::GetMizuPath(Common::FS::MizuPath::KeysDir);

    if (!Common::FS::CreateDir(mizu_keys_dir)) {
        LOG_ERROR(Core, "Failed to create the keys directory.");
    }

    if (Settings::values.use_dev_keys) {
        dev_mode = true;
        LoadFromFile(mizu_keys_dir / "dev.keys", false);
        LoadFromFile(mizu_keys_dir / "dev.keys_autogenerated", false);
    } else {
        dev_mode = false;
        LoadFromFile(mizu_keys_dir / "prod.keys", false);
        LoadFromFile(mizu_keys_dir / "prod.keys_autogenerated", false);
    }

    LoadFromFile(mizu_keys_dir / "title.keys", true);
    LoadFromFile(mizu_keys_dir / "title.keys_autogenerated", true);
    LoadFromFile(mizu_keys_dir / "console.keys", false);
    LoadFromFile(mizu_keys_dir / "console.keys_autogenerated", false);
}
...
void KeyManager::WriteKeyToFile(KeyCategory category, std::string_view keyname,
    ...
    const auto path = mizu_keys_dir / filename;
    ...
    if (add_info_text) {
        void(file.WriteString(
            "# This file is autogenerated by Mizu\n"
...
bool KeyManager::KeyFileExists(bool title) {
    ...
    if (title) {
        return Common::FS::Exists(mizu_keys_dir / "title.keys");
    }

    if (Settings::values.use_dev_keys) {
        return Common::FS::Exists(mizu_keys_dir / "dev.keys");
    }

    return Common::FS::Exists(mizu_keys_dir / "prod.keys");
}
...
void KeyManager::PopulateTickets() {
    ...
    const auto system_save_e1_path =
        Common::FS::GetMizuPath(Common::FS::MizuPath::NANDDir) / "system/save/80000000000000e1";

    const Common::FS::IOFile save_e1{system_save_e1_path, Common::FS::FileAccessMode::Read,
                                     Common::FS::FileType::BinaryFile};

    const auto system_save_e2_path =
        Common::FS::GetMizuPath(Common::FS::MizuPath::NANDDir) / "system/save/80000000000000e2";
